...
BEGIN
	...
	ALTER TABLE fact.GLTransaction DROP CONSTRAINT FK_GLTransaction_GLTransactionPostingDate;
	...
	SELECT
		...
		GLTransactionPostingDate = GLTrans.POSTDATE,
	...
	SELECT
		...
		GLTransactionEffectiveDateKey = ISNULL(EffDt.CalendarKey, -1),
		GLTransactionPostingDateKey = ISNULL(PostDt.CalendarKey, -1),
	...
	FROM
		...
		dim.Branch Brn
			...
			LEFT JOIN
		dim.Calendar EffDt
			ON Trans.GLTransactionEffectiveDate = EffDt.CalendarDate
			LEFT JOIN
		dim.Calendar PostDt
			ON Trans.GLTransactionPostingDate = PostDt.CalendarDate
			LEFT JOIN
	...
	ALTER TABLE fact.GLTransaction ADD CONSTRAINT FK_GLTransaction_GLTransactionPostingDate
	FOREIGN KEY (GLTransactionPostingDateKey) REFERENCES dim.Calendar (CalendarKey);
