...
IF EXISTS(SELECT * FROM sys.procedures WHERE SCHEMA_NAME(schema_id) = 'etl' AND name = 'uspFact_BalanceSheet_PartialRefresh')
	DROP PROCEDURE etl.uspFact_BalanceSheet_PartialRefresh;
GO


CREATE PROCEDURE etl.uspFact_BalanceSheet_PartialRefresh
AS
BEGIN

	DECLARE @CutoffDateInt	INT;
	SET @CutoffDateInt = CAST(CONVERT(VARCHAR(8), DATEADD(dd, -60, SYSDATETIME()), 112) AS INT);
	...
	FROM
		...
		dim.GLAccount GLAcct
			...
			LEFT JOIN
		fact.BalanceSheet Fact
			ON ISNULL(GLAcct.GLAccountKey, -1) = Fact.GLAccountKey AND CAST(CONVERT(VARCHAR(8), Trans.PeriodDate, 112) AS INT) = Fact.PeriodKey
	WHERE
		Fact.BalanceSheetKey IS NULL
		AND
	...
	(
		...
		FROM
			fact.BalanceSheet Bal
				INNER JOIN
			dim.Calendar Dt
				ON Bal.PeriodDateKey = Dt.CalendarKey
		GROUP BY
			Bal.GLAccountKey,
			Bal.GLAccountBranchKey,
