...
IF EXISTS (SELECT * FROM sys.objects WHERE schema_id = SCHEMA_ID('dbo') AND name = 'USEagle_extract_MarquisMCIF_Loans')
	DROP PROCEDURE dbo.USEagle_extract_MarquisMCIF_Loans;
GO


CREATE PROCEDURE dbo.USEagle_extract_MarquisMCIF_Loans
...
BEGIN
	...
	SELECT
		Val = LEFT(RTRIM(Mem.PrimaryMemberFullName) + SPACE(75), 75)											--	Name #1 (Primary Member Full Name) (75, 1-75)
			...
			+ RIGHT(SPACE(11) + CAST(CAST(Ln.LoanOriginalBalance AS DECIMAL(12, 2)) AS VARCHAR), 15)			--	Original Balance (15, 512 - 526) (right-justified)
			+ RIGHT(SPACE(11) + CAST(CAST(Ln.LoanCurrentBalance AS DECIMAL(12, 2)) AS VARCHAR), 15)				--	Current Balance (15, 527 - 541) (right-justified)
			+ SPACE(3)																							--	Term (3, 542 - 544)
			+ RIGHT(SPACE(3) + CAST(CAST(100 * Ln.LoanCurrentRate AS DECIMAL(7, 4)) AS VARCHAR), 7) + '%'		--	Rate (8, 545 - 556) (right-justified)
			+	CASE
