...
IF EXISTS (SELECT * FROM sys.objects WHERE schema_id = SCHEMA_ID('dbo') AND name = 'USEagle_extract_MarquisMCIF_Checking')
	DROP PROCEDURE dbo.USEagle_extract_MarquisMCIF_Checking;
GO


CREATE PROCEDURE dbo.USEagle_extract_MarquisMCIF_Checking
...
BEGIN
	...
	SELECT
		Val = LEFT(RTRIM(Mem.PrimaryMemberFullName) + SPACE(75), 75)											--	Name #1 (Primary Member Full Name) (75, 1-75)
			...
			+ LEFT(RTRIM(LnDesc.ShareAccountNumber) + SPACE(16), 16)											--	Account Number (16, 392 - 407)
			...
			+ RIGHT(SPACE(11) + CAST(CAST(Sh.ShareOriginalBalance AS DECIMAL(12, 2)) AS VARCHAR), 15)			--	Original Balance (15, 512 - 526) (right-justified)
			+ RIGHT(SPACE(11) + CAST(CAST(Sh.ShareCurrentBalance AS DECIMAL(12, 2)) AS VARCHAR), 15)			--	Current Balance (15, 527 - 541) (right-justified)
			+ SPACE(3)																							--	Term (3, 542 - 544)
			+ RIGHT(SPACE(3) + CAST(CAST(ShDtl.DIVRATE / 1000 AS DECIMAL(7, 4)) AS VARCHAR), 7) + '%'			--	Rate (8, 545 - 556) (right-justified)
			+	CASE
	...
	FROM
		...
		USEagleDW.dim.Branch Brn
			...
			INNER JOIN
		USEagleDW.dim.ShareDescriptor LnDesc
			ON Sh.ShareDescriptorKey = LnDesc.ShareDescriptorKey
			INNER JOIN
		...
		ARCUSYM000.dbo.SAVINGS ShDtl
			ON Mem.MemberNumber = ShDtl.PARENTACCOUNT And LnDesc.ShareID = ShDtl.ID AND ShDtl.ProcessDate = @ProcessDateInt
			LEFT JOIN
		JointAccounts Sec
	...
	WHERE
		Typ.ProductCategory2 = 'Share Drafts'
	ORDER BY
		Val;
