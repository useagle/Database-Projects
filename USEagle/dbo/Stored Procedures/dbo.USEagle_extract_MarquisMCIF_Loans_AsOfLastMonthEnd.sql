...
IF EXISTS (SELECT * FROM sys.objects WHERE schema_id = SCHEMA_ID('dbo') AND name = 'USEagle_extract_MarquisMCIF_Loans_AsOfLastMonthEnd')
	DROP PROCEDURE dbo.USEagle_extract_MarquisMCIF_Loans_AsOfLastMonthEnd;
GO


CREATE PROCEDURE dbo.USEagle_extract_MarquisMCIF_Loans_AsOfLastMonthEnd

-- ======================================================================
-- Author:		<Chris Hyde>
-- Create date: <05/08/2019>    
-- Modify Date: 
-- Description:	<extract of loan data for Marquis MCIF>
-- ======================================================================
...
BEGIN
	...
	SELECT
		Val = LEFT(RTRIM(Mem.PrimaryMemberFullName) + SPACE(75), 75)											--	Name #1 (Primary Member Full Name) (75, 1-75)
			...
			+ LEFT(RTRIM(LnDesc.LoanAccountNumber) + SPACE(16), 16)												--	Account Number (16, 392 - 407)
			...
			+	CASE
					WHEN Ln.AccountTypeKey = -1 THEN SPACE(5)
			...
			+	CASE
					WHEN Typ.ProductCategory2 = 'Business' THEN 'B'
					ELSE 'R'
				END																								--	Business or Retail Flag (1, 443 - 443)
			+	CASE
					WHEN Ln.LoanOpenDateKey = -1 THEN SPACE(10)
					ELSE CONVERT(VARCHAR(10), CONVERT(DATE, CAST(Ln.LoanOpenDateKey AS VARCHAR), 112), 101)
				END																								--	Open Date (10, 444 - 453)
			+	CASE
					WHEN Ln.LoanScheduledCloseDateKey = -1 THEN SPACE(10)
					ELSE CONVERT(VARCHAR(10), CONVERT(DATE, CAST(Ln.LoanScheduledCloseDateKey AS VARCHAR), 112), 101)
				END																								--	Maturity Date (10, 454 - 463)
			+	CASE
					WHEN LnDtl.ACTIVITYDATE IS NULL THEN SPACE(10)
					ELSE CONVERT(VARCHAR(10), LnDtl.ACTIVITYDATE, 101)
				END																								--	Last Transaction Date (10, 464 - 473)
			+ SPACE(10)																							--	Original Certificate Open Date (10, 474 - 483)
			+ SPACE(10)																							--	Last Renewal Date (10, 484 - 493)
			+ SPACE(10)																							--	Next Renewal Date (10, 494 - 503)
			...
			+ RIGHT(SPACE(11) + CAST(CAST(ISNULL(LnDtl.ORIGINALBALANCE, 0) AS DECIMAL(12, 2)) AS VARCHAR), 15)	--	Original Balance (15, 512 - 526) (right-justified)
			+ RIGHT(SPACE(11) + CAST(CAST(ISNULL(LnDtl.BALANCE, 0) AS DECIMAL(12, 2)) AS VARCHAR), 15)			--	Current Balance (15, 527 - 541) (right-justified)
			+ SPACE(3)																							--	Term (3, 542 - 544)
			+ RIGHT(SPACE(3) + CAST(CAST(ISNULL(LnDtl.INTERESTRATE, 0) / 1000.0 AS DECIMAL(7, 4)) AS VARCHAR), 7) + '%'			--	Rate (8, 545 - 556) (right-justified)
			+	CASE
			...
			+	CASE
					WHEN Ln.LoanChargeOffDateKey < 0 THEN 'N'
					ELSE 'Y'
				END																								--	Charge-Off Flag (1, 570 - 570)
			+	CASE
					WHEN Ln.LoanChargeOffDateKey < 0 THEN SPACE(10)
					ELSE CONVERT(VARCHAR(10), CONVERT(DATE, CAST(Ln.LoanChargeOffDateKey AS VARCHAR), 112), 101)
			...
			+ CAST(ISNULL(LnDtl.PAYMENTHISTORY3, 0) AS VARCHAR(5))												--	31-60 Day Delinquency (5, 583 - 587)
			+ CAST(ISNULL(LnDtl.PAYMENTHISTORY4, 0) AS VARCHAR(5))												--	61-90 Day Delinquency (5, 588 - 592)
			+ CAST(ISNULL(LnDtl.PAYMENTHISTORY5, 0) AS VARCHAR(5))												--	91-120 Day Delinquency (5, 593 - 597)
			+	CASE
	...
	FROM
		...
		USEagleDW.dim.Branch Brn
			...
			INNER JOIN
		USEagleDW.dim.LoanDescriptor LnDesc
			ON Ln.LoanDescriptorKey = LnDesc.LoanDescriptorKey
			INNER JOIN
		...
		MailingAddress Addr
			...
			LEFT JOIN
		ARCUSYM000.dbo.LOAN LnDtl
			ON Mem.MemberNumber = LnDtl.PARENTACCOUNT And LnDesc.LoanID = LnDtl.ID AND LnDtl.ProcessDate = @ProcessDateInt
			LEFT JOIN
		JointAccounts Sec
